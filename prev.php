...
<form id="form2" name="form2" method="post" action="">
  ...
  <?php 
	...
	if(isset($_POST['loc'])) 
		{
			$nome=$_POST['cmbfor'];
			$sql=mysql_query("SELECT * FROM `forne_terc` WHERE emp='$nome'");
...
<input name="nf_prev" type="text" id="nf_prev" value="<?php echo $NF; ?>" size="15"/>
  ...
  <p><em>
    ...
    <select name="cmbnf" id="cmbnf">
     <?php
      $sql = mysql_query("SELECT  id_nf,cnpj FROM prev where cnpj='$cnpj'");
      if(mysql_num_rows($sql))
		{
			 while($monta = mysql_fetch_assoc($sql))
				  {
		 ...
		 if(isset($_POST['Vi'])) 
			{
				if(empty($_POST['cmbnf'])){
					echo "<b><font color=red> Não existe nota fiscal a ser visulizada ou impressa!!!!  </font></b>";
						...
						{
								$sql=mysql_query("SELECT * FROM `prev` WHERE id_nf='$link'");
									if(mysql_num_rows($sql))
									{
										while($array = mysql_fetch_row($sql)) 
										{		
