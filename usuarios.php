	...
	if(isset($_POST['loc'])) 
  		{
			...
			$query = mysql_query("SELECT * FROM login where Nome='$usuario'");
...
?>
  <p align="ledt">
   ...
   <div class="alert alert-primary btn-sm" role="alert">
    <h5 align="left">CADASTRO DE USUARIOS</h5>
    <strong></strong></div>
  <table width="1109" border="0">
    <tbody>
      <tr>
        <th width="1099" height="34" align="left" scope="col">
		<h6>USUARIO:
          ...
          <?php
      $sql = mysql_query("SELECT * FROM depart where cod_dep='$depart'");
		if(mysql_num_rows($sql))
			{
				while($monta = mysql_fetch_row($query)) 
					{
						$dep_nome = $monta['dep'];
					}		
			}	
  ...
  <h6>
    <input type="submit" name="salve" id="salve" class="btn btn-secondary btn-sm" value="Salvar" tabindex="5" />
      ...
      <input type="submit" name="loc" id="loc" class="btn btn-secondary btn-sm" value="Localizar" tabindex="7" />
      <select name="usuercmd" id="usuercmd"  tabindex="6">
        <?php
		...
		$query = mysql_query("SELECT * FROM login where Nome='$usuario'");
			if(mysql_num_rows($query))
			{
				if($usuario <> "Administrador"){
					$query = mysql_query("DELETE FROM `login` WHERE nome='$usuario'")or die(mysql_error());
 ...
 if(isset($_POST['salve'])) 
  	{
		if(empty( $_POST['nome']) || empty($_POST['senha']) || empty( $_POST['niv']))
			...
			else
			{
				...
				$per= $_POST['niv'];
				$dep =$_POST['depart'];
				$sql = mysql_query("SELECT cod_dep FROM depart where dep='$dep'");
	  			while($monta = mysql_fetch_assoc($sql))
	  			{
        			$dep = $monta['cod_dep'];
      				}
				...
				else
				{
					...
					if($nivel <> 2){
						 ...
						 $sql = mysql_query("SELECT Codigo FROM login ORDER BY Codigo DESC LIMIT 1 ");
						 if(mysql_num_rows( $sql))
							{
...
<div class="collapse" id="adm">
  ...
  <p>
    <?php
    $query = mysql_query( "SELECT `Nome`,`Nivel`,`dep`FROM `login` WHERE 1" );
    $depart = "";
    $ext = "";
    if ( mysql_num_rows( $query ) ) {
	  ...
	  ?>
    <?php
    $sql = mysql_query( "SELECT * FROM `depart` WHERE 1" );
    if ( mysql_num_rows( $sql ) ) {
    $estilos[] = "text-align: center; background-color: rgba(90,96,113);font-size:14px;color:white;";
	$estilos[] = "text-align: center;font-size:14px; font-style: oblique;";
	$estilos[] = "text-align: center;font-size:14px; width: 200%;";
